...
const fillCart = async (req, res) => {
  try {
    ...
    if (!shirt) {
      return res
        .status(400)
        .json({ success: false, message: 'Shirt not found' });
    }
...
const deleteCart = async (req, res) => {
  try {
    const user = res.locals.decodedToken.userId;
    await Cart.findOneAndDelete({ owner: user });
...
module.exports = { fillCart, getCart, editCart, deleteCartItem, deleteCart };
